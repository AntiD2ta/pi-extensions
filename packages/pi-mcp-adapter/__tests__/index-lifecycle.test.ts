...
import { MCP_VISUAL_PROFILE_PRESENTATION_EVENT } from "../visual-profile-presentation.ts";
...
function createPi(options: { unregisterTool?: false | ((name: string) => boolean) } = {}) {
  ...
  return {
    ...
    api: {
      ...
      events: {
        on: vi.fn((event: string, handler: (payload: unknown) => unknown) => eventHandlers.set(event, handler)),
        emit: vi.fn(),
      },
      getAllTools: vi.fn(() => []),
...
describe("mcpAdapter session lifecycle", () => {
  ...
  it("updates and restores proxy, direct, and script renderers when a profile acquires presentation", async () => {
    mocks.loadMcpConfig.mockReturnValue({
      settings: { toolResultRendering: "boxed" },
    ...
    const { default: mcpAdapter } = await import("../index.ts");
    const { api, eventHandlers } = createPi();
    mcpAdapter(api);
    const presentation = eventHandlers.get(MCP_VISUAL_PROFILE_PRESENTATION_EVENT);
    ...
    const acquire = {
      ...
      profile: { style: "boxed" as const, borderStyle: "rounded" as const, glyphMode: "unicode" as const, padding: 1 as const },
    };
    presentation!(acquire);
    expect(acquire.result).toMatchObject({ supported: true, accepted: true });

    for (const name of ["mcp", "demo_search", "mcpScript"]) {
      const registrations = api.registerTool.mock.calls
        .map((call: any[]) => call[0])
        .filter((tool: { name: string }) => tool.name === name);
      expect(registrations.at(-1)).toMatchObject({ renderShell: "self", renderResult: expect.any(Function) });
    }

    const release = { version: 1 as const, owner: "pi-visual-profile", action: "release" as const };
    presentation!(release);
    expect(release.result).toMatchObject({ supported: true, accepted: true });
    for (const name of ["mcp", "demo_search", "mcpScript"]) {
      const registrations = api.registerTool.mock.calls
        .map((call: any[]) => call[0])
        .filter((tool: { name: string }) => tool.name === name);
      expect(registrations.at(-1)).toMatchObject({ renderShell: "default" });
    }
  });
