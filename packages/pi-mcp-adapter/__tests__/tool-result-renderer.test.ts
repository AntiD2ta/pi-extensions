...
describe("MCP tool result renderer", () => {
  ...
  it("keeps the adapter call renderer hidden when a profile card is expanded", () => {
    const call = createMcpProxyToolCallRenderer({
      resultRendering: "compact",
      collapsedResultLines: 1,
      visualProfile: { style: "boxed", borderStyle: "rounded", glyphMode: "unicode", padding: 1 },
    })({ tool: "list_projects", server: "plane" }, plainTheme, { isError: false, isPartial: false, expanded: true });

    expect(call.render(120)).toEqual([]);
  });

  it("keeps the mcpScript identity without copying script code into profile cards", () => {
    const state: { compactTitle?: string; compactInputPreview?: string } = {};
    const code = 'emit("secret-script-code");';
    const call = createMcpScriptToolCallRenderer({
      resultRendering: "compact",
      collapsedResultLines: 1,
      visualProfile: { borderStyle: "rounded", glyphMode: "unicode", padding: 1 },
    })(
      { code },
      plainTheme,
      { isError: false, isPartial: false, expanded: false, state },
    ...
    const output = renderMcpToolResult(
      result([{ type: "text", text: "done" }], { server: "plane", tool: "get_work_item" }),
      collapsedOptions,
      plainTheme,
      { isError: false, state },
      {
    ...
    expect(call.render(120)).toEqual([]);
    expect(state.compactTitle).toBe("mcpScript");
