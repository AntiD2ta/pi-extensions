...
describe("syncNamespaceProxyTools", () => {
  ...
  it("uses the adapter-owned profile renderer for namespace proxies", async () => {
    const { syncNamespaceProxyTools } = await importSync();
    const { pi, registered } = makePi();

    syncNamespaceProxyTools({
      config: { mcpServers: { docs: { command: "docs" } } },
      cache: CACHE_SHAPE([["docs", { tools: [{ name: "search" }] }]]),
      envOverride: null,
      existingDirectNames: new Set(),
      existingNamespaceNames: new Set(),
      pi,
    ...
    const tool = registered.get("mcp__docs")!;
    expect(tool.renderShell).toBe("self");
    expect(tool.renderCall!({ tool: "search" }, { fg: (_name: string, text: string) => text }, { isError: false, isPartial: false, state: {} }).render(80)).toEqual([]);
