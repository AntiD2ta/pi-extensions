...
import { renderVisualProfileMcpResult, type VisualProfilePresentation } from "./visual-profile-renderer.ts";
...
export interface McpToolRenderOptions {
  ...
  visualProfile?: VisualProfilePresentation;
}
...
function shouldUseCompactFinalRender(options: McpToolRenderOptions, context?: McpToolRenderContext): boolean {
  if (options.visualProfile && context?.isPartial === false) return true;
...
export function createMcpScriptToolCallRenderer(options = resolveMcpToolRenderOptions()) {
  return (args: { code: string }, theme?: RenderTheme, context?: McpToolRenderContext) => {
    if (context?.state) {
      context.state.compactTitle = "mcpScript";
      delete context.state.compactInputPreview;
    }
    if (shouldUseCompactFinalRender(options, context)) return new EmptyComponent();
...
export function renderMcpToolResult(
  ...
  const activeTheme = theme ?? plainTheme;
  if (renderOptions.visualProfile) {
    return renderVisualProfileMcpResult(result, options, activeTheme, renderOptions.visualProfile, {
      ...(context?.state?.compactTitle ? { title: context.state.compactTitle } : {}),
      ...(context?.state?.compactInputPreview ? { inputPreview: context.state.compactInputPreview } : {}),
    });
  }
