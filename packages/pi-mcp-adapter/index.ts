...
import { applyVisualProfilePresentation, MCP_VISUAL_PROFILE_PRESENTATION_EVENT, type McpVisualProfilePresentationRequest } from "./visual-profile-presentation.ts";
...
function installMcpAdapter(pi: ExtensionAPI, options: McpAdapterOptions) {
  ...
  let presentationOwner: string | undefined;
  let presentationProfile: McpVisualProfilePresentationRequest["profile"];
  const scriptModeEnabled = earlyConfig.settings?.scriptMode !== false;
  let toolRenderOptions = resolveMcpToolRenderOptions(earlyConfig.settings);
  let toolRenderShell: "self" | "default" = toolRenderOptions.resultRendering === "compact" ? "self" : "default";
  let renderMcpToolResult = createMcpToolResultRenderer(toolRenderOptions);

  function refreshPresentationRenderers(): void {
    toolRenderOptions = {
      ...resolveMcpToolRenderOptions(earlyConfig.settings),
      ...(presentationProfile ? { visualProfile: presentationProfile } : {}),
    };
    toolRenderShell = presentationProfile || toolRenderOptions.resultRendering === "compact" ? "self" : "default";
    renderMcpToolResult = createMcpToolResultRenderer(toolRenderOptions);
    if (scriptModeEnabled) registerMcpScriptTool();
  }
  ...
  pi.events.on(MCP_VISUAL_PROFILE_PRESENTATION_EVENT, (rawRequest: unknown) => {
    if (typeof rawRequest !== "object" || rawRequest === null || Array.isArray(rawRequest)) return;
    const request = rawRequest as McpVisualProfilePresentationRequest;
    if (request.result !== undefined) return;
    const applied = applyVisualProfilePresentation(presentationOwner, request);
    request.result = applied.result;
    if (!applied.result.accepted || request.action === "query") return;
    presentationOwner = applied.owner;
    presentationProfile = applied.profile;
    refreshPresentationRenderers();
  ...
  function registerMcpScriptTool(): void {
    (pi.registerTool as (tool: unknown) => unknown)({
      name: "mcpScript",
      label: "MCP Script",
