...
// Formats elapsed milliseconds with a compact human-readable label.
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("turn_start", async (_event, ctx) => {
    ...
    const updateMessage = () => {
      const elapsedLabel = formatElapsedLabel(Date.now() - turnStartedAt);
      ctx.ui.setWorkingMessage(`${baseMessage} (${elapsedLabel})`);
    };

    updateMessage();
    updateTimer = setInterval(updateMessage, 1000);
  });
