...
This package is a maintained source copy of MasuRii's [`pi-tool-display` v0.5.0](UPSTREAM.md) for Pi 0.85.x. It supports `@earendil-works/pi-coding-agent` and `@earendil-works/pi-tui` `>=0.85.0 <0.86.0`. See [NOTICE](NOTICE) for attribution and the recorded `zellij-modal.ts` provenance gap.

Install the monorepo checkout with `pi install /absolute/path/to/pi-extensions`. To let `pi-visual-profile` render edit and write cards, set `registerToolOverrides.edit` and `registerToolOverrides.write` to `false`, then run `/reload`. When they remain enabled, `pi-tool-display` owns those built-in renderers.
