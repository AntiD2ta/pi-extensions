...
export function adjustPowerlineEditorMouseEvent(
  event: { x: number; y: number },
  options: { editorChrome: boolean; width: number },
): { x: number; y: number } {
  if (!options.editorChrome || options.width < 10) return event;
...
export default function powerlineFooter(pi: ExtensionAPI) {
  ...
  let welcomeHeaderAdditive = false;
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    if (enabled && ctx.hasUI) {
      ...
      if (shouldShowStartupWelcome(event.reason, config.welcome)) {
        ...
        } else if (config.welcomeMode === "header") {
          setupWelcomeHeader(ctx, true);
        } else {
  ...
  function dismissWelcome(ctx: any) {
    ...
    if (welcomeHeaderActive) {
      welcomeHeaderActive = false;
      if (welcomeHeaderAdditive) {
        const ui = ctx.ui as typeof ctx.ui & { setHeaderWidget?: (key: string, factory: undefined) => void };
        ui.setHeaderWidget?.("powerline-welcome", undefined);
      } else {
        ctx.ui.setHeader(undefined);
      }
      welcomeHeaderAdditive = false;
    }
  }
  ...
  function setupWelcomeHeader(ctx: any, additive = false) {
    ...
    welcomeHeaderAdditive = additive;
    ...
    if (additive) {
      const ui = ctx.ui as typeof ctx.ui & { setHeaderWidget?: (key: string, factory: unknown) => void };
      ui.setHeaderWidget?.("powerline-welcome", factory);
    } else {
