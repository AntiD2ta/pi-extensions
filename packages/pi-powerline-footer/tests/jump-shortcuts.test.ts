...
test("Powerline editor click coordinates exclude its prompt gutter", () => {
  assert.deepEqual(
    adjustPowerlineEditorMouseEvent({ x: 9, y: 1 }, { editorChrome: true, width: 80 }),
    { x: 6, y: 1 },
  );
  assert.deepEqual(
    adjustPowerlineEditorMouseEvent({ x: 9, y: 1 }, { editorChrome: false, width: 80 }),
    { x: 9, y: 1 },
  );
  assert.deepEqual(
