...
test("welcome header mode keeps Pi's startup header visible", () => {
  const config = parsePowerlineConfig({ welcome: "header" }, ["default"] as const);

  assert.equal(config.welcome, true);
  assert.equal(config.welcomeMode, "header");
  assert.match(indexSource, /setHeaderWidget\?\.\("powerline-welcome"/);
});
