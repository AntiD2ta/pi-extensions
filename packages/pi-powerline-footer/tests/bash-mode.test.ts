...
import { codingAgentModuleUrl, piTuiModuleUrl } from "./pi-modules.ts";
...
test("bash editor does not submit pasted multiline input while bracketed paste is active", async () => {
  const { BashModeEditor } = await import("../bash-mode/editor.ts");
  const { CustomEditor } = await import(codingAgentModuleUrl("modes/interactive/components/custom-editor.js"));
...
test("bash editor fast path preserves plain custom keybindings", async () => {
  const { BashModeEditor } = await import("../bash-mode/editor.ts");
  const { KeybindingsManager } = await import(codingAgentModuleUrl("core/keybindings.js"));
  const { getKeybindings, setKeybindings } = await import(piTuiModuleUrl("dist/index.js"));
