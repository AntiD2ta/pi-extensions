...
test("stash shortcut matches Cmd+B without consuming the retired Alt+S encodings", () => {
  assert.equal(matchesStashShortcutInput("\x1b[98;9u"), true);
  assert.equal(matchesStashShortcutInput("\x1b[98;9:2u"), true);
...
test("agent_end leaves an active stash untouched until explicit restore", async () => {
  ...
  try {
    ...
    runtime.sendTerminalInput("\x1b[98;9u");
